...
import type { TicketCommit, TicketDbChange } from "../types";
...
function distribuirMinutos(commits: TicketCommit[], total: number): number[] {
	...
	const explicit = commits.map((c) => c.minutos);
	if (explicit.every((m) => m != null && m > 0)) {
	...
	if (diff !== 0) {
		const idx = tiempos.reduce((best, t, i, arr) => (t > arr[best] ? i : best), 0);
		tiempos[idx] = Math.max(1, tiempos[idx] + diff);
	}
