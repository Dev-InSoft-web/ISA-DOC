...
const MERMAID_ZENUML_CDN =
	"https://cdn.jsdelivr.net/npm/@mermaid-js/mermaid-zenuml@0.1.2/dist/mermaid-zenuml.esm.min.mjs";
...
export async function ensureMermaid(): Promise<MermaidAPI> {
	...
	const promise = (async (): Promise<MermaidAPI> => {
		...
		try {
			const zenumlMod = await import(/* @vite-ignore */ MERMAID_ZENUML_CDN);
			const zenumlPkg = (zenumlMod as { default?: unknown }).default ?? zenumlMod;
			const reg = mermaid as MermaidAPI & {
				registerExternalDiagrams?: (diagrams: unknown[]) => Promise<void>;
			};
			await reg.registerExternalDiagrams?.([zenumlPkg]);
		} catch {
			// zenuml opcional en visor local
		}
		try {
			mermaid.initialize({
				startOnLoad: false,
