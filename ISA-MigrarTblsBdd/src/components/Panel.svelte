...
{:else}
	<Card variant="flat">
		<FlexLayout items="center">
			...
			<Button variant="outlined" onClick={loadDbStatus}>Refrescar</Button>
		</FlexLayout>
	</Card>
	...
	{#if logs.length > 0}
		<section class="section">
			<FlexLayout items="center" gap="0.5rem">
				<Button variant="outlined" onClick={() => (showLogsModal = true)}>
					<Iconify icon="mdi:history" />
					<span>Log de ejecución ({logsOk}/{logs.length}{logsErr ? " • " + logsErr + " err" : ""})</span>
				</Button>
			</FlexLayout>
		</section>
