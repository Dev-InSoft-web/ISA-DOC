...
/** Tope si la API pide una espera larga (ASPH Groq, reset diario, etc.). */
...
/** @deprecated Alias — usar RateLimitHintTracker */
...
/** Cabeceras Cerebras: RPD/RPM/TPM y segundos hasta reset. */
function parseRateLimitHeaders(headers: Record<string, string>, tracker: RateLimitHintTracker): void {
	...
	for (const [sec, label] of [
		...
		if (sec != null) {
			const ms = Math.ceil(sec * 1000) + 1500;
			bumpWait(tracker, ms, `${label} ${sec}s`);
		}
	}
...
/** Registra pistas de espera y cupos consumidos (texto de error + cabeceras HTTP). */
...
/** Mayor espera inferida del intento (varios proveedores / keys / chunks). */
...
/**
 * Espera antes de reintentar Whisper: pistas API (RPD/TPM/ASPH) + escalonado por intento
 * para no saturar Groq con decenas de reintentos seguidos.
 */
export function waitMsForWhisperRetry(params: {
	attempt: number;
	errorMessage: string;
	tracker: RateLimitHintTracker;
	/** Promedio histórico de esperas 429 (whisper-stats). */
...
export function formatRateLimitHintSummary(tracker: RateLimitHintTracker): string {
	...
	const waitSec = Math.round(tracker.maxMs / 1000);
	return `${parts.join(" · ")} → espera ${waitSec}s`;
}
...
/** Espera 429/límites según API (sin tope de 60s). */
...
/** Cabeceras en errores del SDK Cerebras / OpenAI. */
...
export function logProgress(...args: unknown[]): void {
	console.log(...args);
}
