...
function sourceExprFromRule(rule) {
  if (!rule) return null;
  if (/^[A-Za-z_][A-Za-z0-9_]*$/.test(rule)) return `s.[${rule}]`;
  return rule;
}

export function buildColumnPlan(destColumns, sourceColumns, tableManualMapping = {}) {
  ...
  for (const destCol of destColumns) {
    ...
    let selected = srcByName.get(destKey) || null;
    let strategy = "exact";

    if (!selected) {
      selected = srcByNorm.get(norm(destName)) || null;
      strategy = selected ? "normalized" : strategy;
    }
