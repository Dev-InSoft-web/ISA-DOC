...
const TICKETS_DIR = path.join(ROOT, "src", "lib", "features", "tickets");
const RECORDS_DIR = path.join(TICKETS_DIR, "records");
const ASSETS_DIR = path.join(TICKETS_DIR, "assets");
const PNG_DIR = path.join(ASSETS_DIR, "_meta", "code");
const MAP_FILE = path.join(ASSETS_DIR, "_meta", "code-imgs.json");
...
async function* walkTkFiles(dir) {
	const entries = await fs.readdir(dir, { withFileTypes: true });
	for (const e of entries) {
		const p = path.join(dir, e.name);
		if (e.isDirectory()) yield* walkTkFiles(p);
		else if (/^TK-\d+\.ts$/.test(e.name)) yield p;
	}
}
...
async function discoverSnippets() {
	...
	for await (const filePath of walkTkFiles(RECORDS_DIR)) {
