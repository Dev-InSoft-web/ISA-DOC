...
} from "./youtube-dedupe-segments.ts";
...
function runYtDlp(args: string[], timeoutMs = 180_000): { ok: boolean; stderr: string } {
	const proc = spawnSync("python", ["-m", "yt_dlp", "--no-update", ...args], {
		encoding: "utf8",
		timeout: timeoutMs,
...
export async function fetchVideoBundle(
	...
	if (opts.fetchComments) {
		args.push("--write-comments", "--extractor-args", `youtube:max_comments=${opts.maxComments},all,-30;comment_sort=top`);
	}

	const { ok, stderr } = runYtDlp(args);
	if (!ok) throw new Error(`yt-dlp falló para ${videoId}: ${stderr.slice(0, 500)}`);
	...
	try {
		...
		throw new Error(`No se encontró info.json para ${videoId}`);
	}
...
export async function buildVideoCorpusRecord(
	...
	opts: FetchVideoOptions & { listTitle?: string; transcriptMethod: string },
	...
	return {
		...
		files: {
			md: `videos/${videoId}.md`,
			json: `videos/${videoId}.json`,
			infoJson: `videos/${videoId}.info.json`,
		},
	};
}

/** Guarda JSON estructurado + dump crudo yt-dlp en videos/. */
export async function persistVideoArtifacts(
	...
	const jsonPath = join(videosDir, `${record.videoId}.json`);
	const infoCopy = join(videosDir, `${record.videoId}.info.json`);

	await writeFile(jsonPath, `${JSON.stringify(record, null, 2)}\n`, "utf8");
	try {
		const raw = await readFile(rawInfoPath, "utf8");
		await writeFile(infoCopy, raw, "utf8");
	} catch {
		/* opcional */
	}
}
...
export async function loadCachedRecord(
	...
	try {
		const raw = await readFile(join(videosDir, `${videoId}.json`), "utf8");
