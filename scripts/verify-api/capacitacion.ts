...
export interface CapacitacionParams {
	IDRIVER_TEST: number;
	ATTR_BASE: number;
	IPLAN: string;
	ICURSO: string;
 ...
 */
export async function testCapacitacionVerificarConsolidarRecodificar({
	IDRIVER_TEST,
	ATTR_BASE,
	IPLAN,
	ICURSO,
	iplanestudio,
}: CapacitacionParams): Promise<void> {
	const err = "⛔";
	console.log(`\n###############################################################`);
	console.log(`CAPACITACIÓN: Verificar / duplicar / consolidar / recodificar`);
	console.log(`###############################################################`);

	const verificarCases: Array<[string, string, string]> = [
		["tema", "TEMATEST", "tema"],
	...
	const rDupSrc = await request("POST", "/api/tema", { itema: TEMA_DUP_SRC, ntema: "Tema duplicar origen" });
	...
	if (dupDr.ok && !dupDr.skipped) {
		createdEntities.push({
			...
			idPath1: "299",
			extras: [],
			getIdStr: (o: any) => String(o.idriver),
			icon: "✅",
		});
	}
