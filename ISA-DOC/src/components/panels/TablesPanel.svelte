<script lang="ts">
   import { Button, Card, FlexLayout, H2, H4, Iconify, Text, Toaster, toastError, toastSuccess } from "@ingenieria_insoft/ispsveltecomponents";
   ...
   import { fragmentsStore, refreshFragments, setFragmentsAfterSave, startFragmentsSocket, type SqlFragment, type SqlFragmentKind } from "../../lib/fragmentsStore.ts";
   import type { ParsedTable } from "../../lib/tableSchema.ts";
   import { emitDropTable, emitTable, emitTablesAsBody, parseTableFragment } from "../../lib/tableSchema.ts";
   ...
   function tablePrefix(t: ParsedTable): string {
      return detectPrefix(t.name);
   }
...
<section class="tables-section">
   ...
   {:else}
      ...
      {#each prefixOrder as pkey (pkey)}
         ...
         {#if items.length > 0}
            <AccordionActions title={pkey ? `Prefijo · ${pkey}` : "Sin prefijo"} icon={pkey ? "mdi:label" : "mdi:label-off-outline"} count={items.length} open={false}>
               <Card variant="flat">
                  <FlexLayout direction="column">
                     <FlexLayout items="center">
                        ...
                        <H4>Prefijo</H4>
                        {#if pEntry}
                           <input class="input-field prefix-input" type="text" bind:value={pEntry.current} on:change={(e) => onPrefixInput(pIdx, (e.currentTarget as HTMLInputElement).value)} placeholder="(sin prefijo)" />
                     ...
                     <Card variant="flat">
                        ...
                        <FlexLayout direction="column">
                           {#each catItems as entry (entry.table.fragmentId + "::" + entry.table.originalName)}
                              <AccordionActions inner title={entry.table.name} titleIcon="mdi:table" open={false}>
                                 <SqlTreeEditor table={entry.table} prefix={tablePrefix(entry.table)} onChange={(t) => onTableChange(entry.index, t)} />
                                 ...
                                 {#if cfg}
                                    ...
                                    <AccordionActions inner title="TypeScript" titleIcon="mdi:language-typescript" open={false}>
                                       <div class="cmp-row">
...
<style>
   ...
   .cmp-local {
      border-color: var(--is-success, #2e7d32);
   }
   .cmp-prod {
