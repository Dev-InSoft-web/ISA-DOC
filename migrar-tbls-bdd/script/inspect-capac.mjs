 ...
 *
 * Uso:
 *   node script/inspect-capac.mjs --mode list-aplic-tables
 *   node script/inspect-capac.mjs --mode columns --tables CAPAC_PLANDECURSO_OLD,CAPAC_PLANES_CURSOS
 *   node script/inspect-capac.mjs --mode sample-iplan --top 12
 */
...
function parseArgs(argv) {
  ...
  return {
    mode: modeIdx >= 0 ? argv[modeIdx + 1] : "list-aplic-tables",
    tables:
      tablesIdx >= 0
...
async function listAplicTables(pool) {
...
async function listColumns(pool, tables) {
  const targets =
...
async function sampleIplan(pool, top) {
