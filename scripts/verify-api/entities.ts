import { createdEntities } from "./state.ts";
import { request, requestListGet } from "./http.ts";
import { unwrap, checkCoherence, listContainsAllPks, filtroB64 } from "./helpers.ts";

type AnyObj = Record<string, any>;

export interface DuplicarResult {
	ok: boolean;
	data: any;
	skipped?: boolean;
}

export interface CrulOptions {
	stripFromExtras?: string[];
	skipCreateCoherence?: boolean;
	afterCreateRead?: ((u: any) => unknown | Promise<unknown>) | null;
	relaxCreateRecoverGet?: boolean;
}

/**
 * CRUD + listado para entidad con PK compuesta (ej. driver/atributo: idriver + iatributo).
 */
export async function testEntityCompositeCRUL(
	name: string,
	apiName: string,
	listApiName: string,
	pkKeys: string[],
	basePk: AnyObj,
	createBodyBase: AnyObj,
	updateBody: AnyObj,
	extrasPks: AnyObj[],
	icon: string = "✅",
): Promise<boolean> {
	...
	const getIdStr = (obj: AnyObj): string => pkKeys.map((k) => obj[k]).join("/");
	...
	for (const pk of extrasPks) {
		const p: AnyObj = { ...createBodyBase, ...pk };
...
export async function testVerificarEndpoint(
	label: string,
	apiName: string,
	idPath: string,
	errorIcon: string = "⛔",
...
export function encabezadoMensaje(data: unknown): string {
	try {
		const u: any = typeof data === "string" ? JSON.parse(data) : data;
 ...
 */
export async function testDuplicarEndpoint(
	label: string,
	apiName: string,
	idPath: string,
	body: AnyObj,
	errorIcon: string = "⛔",
): Promise<DuplicarResult> {
...
export async function testConsolidarEndpoint(
	label: string,
	apiName: string,
	idPath: string,
	body: AnyObj,
...
export async function testRecodificarEndpoint(
	label: string,
	apiName: string,
	idPath: string,
	body: AnyObj,
 ...
 */
export async function testDriverNestedAtributos(
	idriver: number,
	atributoPks: AnyObj[],
	icon: string = "✅",
): Promise<boolean> {
	...
	const ids = new Set(attrs.map((a: any) => a.iatributo));
	...
	if (found) {
		...
		console.error(
			`   Result: faltan iatributo en driver.atributos. Esperados: ${atributoPks.map((p) => p.iatributo).join(",")} ${errorIcon}`,
		);
	}
	...
	const existentes = Array.isArray(attrs)
		...
		: [];
	const atributosMerged = [...existentes.filter((a: any) => a.iatributo !== nuevoAtributo.iatributo), nuevoAtributo];
	const putBody = {
		idriver,
	...
	createdEntities.push({
		...
		getIdStr: (obj: AnyObj) => `${obj.idriver}/${obj.iatributo}`,
		icon: "✅",
	});
...
export async function testEntityCRUL(
	name: string,
	apiName: string,
	pkBaseObj: AnyObj,
	createBodyBase: AnyObj,
	updateBody: AnyObj,
	listApiName: string | null = null,
	icon: string = "✅",
	crulOptions: CrulOptions = {},
): Promise<boolean> {
	console.log(`\n===============================================================`);
	...
	const getIdStr = (obj: AnyObj): string => Object.values(obj).join("/");
	const idKey = Object.keys(pkBaseObj)[0];
	const baseIdVal = pkBaseObj[idKey];
	const isNum = typeof baseIdVal === "number";
	const pk1: AnyObj = { ...pkBaseObj };
	const idPath1 = getIdStr(pk1);

	const extras: AnyObj[] = [1, 2, 3].map((i) => {
		const newId = isNum ? (baseIdVal as number) + i : `${baseIdVal}_${i}`;
	...
	if (res.status === 200 || res.status === 201) {
		...
		} else if (apiName === "plan/estudio" && process.env.VERIFY_API_RELAX_PLANESTUDIO_POST !== "0") {
			console.log(
	...
	for (const ex of extras) {
		const p: AnyObj = { ...createBodyBase, ...ex };
	...
	if (res.status === 200) {
		...
		if (Array.isArray(listData)) {
			const foundIds = listData.map((item: any) => item[idKey]);
			// eslint-disable-next-line eqeqeq
			const missing = allTestIds.filter((tid) => !foundIds.some((fid: any) => fid == tid));
	...
	for (let i = createdEntities.length - 1; i >= 0; i--) {
		...
		if (ent.composite && ent.allPks && ent.getIdStr) {
			for (const pk of ent.allPks) {
				await request("DELETE", `/api/${apiName}/${ent.getIdStr(pk as AnyObj)}`);
			}
		} else {
			const { idPath1, extras, getIdStr } = ent;
			if (idPath1) await request("DELETE", `/api/${apiName}/${idPath1}`);
			if (extras && getIdStr) {
				for (const ex of extras) await request("DELETE", `/api/${apiName}/${getIdStr(ex as AnyObj)}`);
			}
		}
		...
		if (res.status === 200) {
			...
			if (Array.isArray(listData)) {
				...
				if (ent.composite && ent.pkKeys && ent.allPks) {
					stillThere = ent.allPks.filter((pk: AnyObj) =>
						// eslint-disable-next-line eqeqeq
						listData.some((row: any) => ent.pkKeys!.every((k) => row[k] == pk[k])),
					);
				} else {
					...
					if (idKey && allTestIds) {
						const foundIds = listData.map((item: any) => item[idKey]);
						// eslint-disable-next-line eqeqeq
						stillThere = allTestIds.filter((tid) => foundIds.some((fid: any) => fid == tid));
					}
				}
