...
async function actionCheck(pool: sql.ConnectionPool) {
  ...
  console.log("oid:", sets[0]?.[0]?.oid ?? null);
  console.log("rows:", sets[1]?.[0]?.n ?? 0);
  console.log("sample:", sets[2] || []);
}

async function actionDrop(pool: sql.ConnectionPool) {
  await pool
    .request()
...
async function actionCreateMirror(pool: sql.ConnectionPool) {
...
async function actionReseed2Levels(pool: sql.ConnectionPool) {
  await pool.request().query(`
