// V2: bloque por bloque — aplica commits desde __propuesta.json a tickets/index.ts
const fs = require("fs");
const path = require("path");

const ROOT = path.resolve(__dirname, "..", "..");
const props = JSON.parse(fs.readFileSync(path.join(ROOT, "__propuesta.json"), "utf8"));
const target = path.join(ROOT, "src/lib/features/tickets/index.ts");
