<script lang="ts">
	...
	// CategorÃ­as relevantes para extraer datos de tablas _OLD a nuevas tablas
	const MIGRATION_CATEGORIES = new Set(["AnÃ¡lisis", "MigraciÃ³n"]);
	...
	function extractSql(text: string): string {
		// Mantiene el output completo â€” el usuario puede editarlo en el modal
		// antes de ejecutar. Se quita prefijo comÃºn de logger "[xx] " si existe.
	...
	function generateSql(action: ScriptAction): void {
		...
		socket?.emit("script:exec", { id: action.id, params }, (data: { ok: boolean; output?: string; error?: string }) => {
			...
			logs = [{
				id: action.id,
				actionName: `Generar SQL Â· ${action.name}`,
			...
			if (ok) {
				...
				toastSuccess(`SQL generado y guardado Â· ${action.name}`);
				codeModalTitle = `SQL generado Â· ${action.name}`;
				codeModalValue = sql;
				codeModalLanguage = "sql";
				codeModalShow = true;
			} else {
				toastError(`GeneraciÃ³n fallÃ³ Â· ${action.name}`);
			}
		});
	}
	...
	function openExecuteModal(action: ScriptAction): void {
		...
		if (!sql.trim()) {
			toastError(`Sin SQL guardado. Use Â«Generar SQLÂ» primero.`);
			return;
		}
		execModalActionId = action.id;
		execModalTitle = `Ejecutar SQL guardado Â· ${action.name}`;
		execModalSql = sql;
		execModalShow = true;
	}
	...
	function confirmExecuteSavedSql(): void {
		if (!execModalSql.trim()) { toastError("SQL vacÃ­o"); return; }
		const confirmed = confirm(`âš ï¸ Se va a ejecutar el SQL guardado contra la BD.\n\nÂ¿Continuar?`);
		if (!confirmed) return;
		...
		socket?.emit("sql:exec", { sql: execModalSql }, (data: { ok: boolean; output?: string; error?: string }) => {
			...
			logs = [{
				...
				actionName: `Ejecutar SQL guardado Â· ${execModalActionId}`,
				timestamp: new Date().toLocaleTimeString(),
				ok,
				output,
			}, ...logs];
			...
			if (ok) {
				...
				toastError("EjecuciÃ³n SQL fallÃ³");
			}
		});
	}
	...
	function executeAction(action: ScriptAction): void {
		if (action.severity === "danger") {
			const confirmed = confirm(`âš ï¸ ACCIÃ“N DESTRUCTIVA\n\n${action.name}\n${action.description}\n\nÂ¿Continuar?`);
			if (!confirmed) return;
		}
		runningId = action.id;
		...
		socket?.emit("script:exec", { id: action.id, params }, (data: { ok: boolean; output?: string; error?: string }) => {
			...
			else toastError(`${action.name} fallÃ³`);
			const output = data.output ?? data.error ?? "Sin salida";
			logs = [{
				id: action.id,
	...
	function execSqlViaSocket(sql: string): Promise<{ ok: boolean; output?: string; error?: string }> {
		return new Promise((resolve) => {
			if (!socket) { resolve({ ok: false, error: "Sin conexiÃ³n al servidor" }); return; }
			socket.emit("sql:exec", { sql }, (data: { ok: boolean; output?: string; error?: string }) => {
				logs = [{
					id: "permisos-csv",
					actionName: "MigraciÃ³n PERMISOS desde CSV",
					timestamp: new Date().toLocaleTimeString(),
					ok: data.ok ?? false,
...
{:else}
	<Card variant="flat">
		<FlexLayout items="center">
			...
			<div>
				<H4>MigraciÃ³n de datos Â· CAPAC_*_OLD â†’ tablas nuevas</H4>
				<Text color="neutral"><small>Extrae registros desde las tablas legacy <code>CAPAC_*_OLD</code> y los inserta en las tablas destino. Use Â«AnÃ¡lisisÂ» para inspeccionar antes y Â«MigraciÃ³nÂ» para generar y ejecutar el SQL.</small></Text>
			</div>
		</FlexLayout>
	</Card>
	...
	<Card variant="flat">
		<FlexLayout items="center">
			...
			<div>
				<H4 color={dbStatus?.ok ? "success" : "error"}>ConexiÃ³n a BD</H4>
				{#if dbStatus?.ok}
					<Text color="color">
						<strong>{dbStatus.database}</strong> â€” {dbStatus.tables?.length ?? 0} tablas CAPAC
					</Text>
				{:else}
					<Text color="error">{dbStatus?.error ?? "Sin conexiÃ³n"}</Text>
				{/if}
			</div>
			...
			<FlexLayout items="center">
				...
				{#if logs.length > 0}
					<Button variant="outlined" onClick={() => (showLogsModal = true)} style="width: fit-content;">
						...
						<span>Log ({logsOk}/{logs.length}{logsErr ? " â€¢ " + logsErr + " err" : ""})</span>
					</Button>
				{/if}
			</FlexLayout>
		</FlexLayout>
		...
		<Modal bind:bshow={showLogsModal} onClose={() => (showLogsModal = false)} style="width: 90dvw; height: 80dvh;">
			<svelte:fragment slot="title">
				<FlexLayout items="center" gap="0.4rem">
					<Iconify icon="mdi:history" />
					<Text><strong>Log de ejecuciÃ³n</strong> <small style="opacity: 0.7;">({logsOk} ok / {logsErr} err / {logs.length} total)</small></Text>
...
<Modal bind:bshow={execModalShow} onClose={() => (execModalShow = false)} style="width: 90dvw; height: 80dvh;">
	...
	<div class="exec-wrap">
		<Text color="neutral"><small>SQL persistido en <code>localStorage["migration-sql:{execModalActionId}"]</code>. PuÃ©deslo editar antes de ejecutar.</small></Text>
