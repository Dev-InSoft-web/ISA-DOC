<script lang="ts">
	...
	const ICONIFY_ALL: ProjectAction = {
		...
		command: [
			...
			`npm --prefix "${base}/ClientesIS/doc/ISA-MigrarTblsBdd" run iconify`,
		].join(" && "),
		cwd: `${base}/ClientesIS`,
...
{#if loading}
	<Loading bShow={true} />
{:else}
	<!-- Botón general de iconos: fuera de los tabs (aplica a todo) -->
...
<style>
	...
	.general-block :global(.group-card) {
		padding: 0.75rem;
		height: 100%;
	}
