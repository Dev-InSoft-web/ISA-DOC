...
WITH req AS (
  ...
  FROM (VALUES
    ...
    (N'TRES_COLUMNAS', CAST(3 AS BIGINT), N'driver-de-video'),
    ...
    (N'SEC_VIDEOS', CAST(3 AS BIGINT), N'driver-de-video'),
    ...
    (N'SEC_RELACIONADOS', CAST(3 AS BIGINT), N'driver-de-video'),
...
src_attr AS (
  ...
  INNER JOIN dbo.CAPAC_ATRIBUTOS_X_DRIVERS axd
   ...
   AND LOWER(LTRIM(RTRIM(axd.NATRIBUTO))) = N'driver-de-video'
)
...
src_attr AS (
  ...
  INNER JOIN dbo.CAPAC_ATRIBUTOS_X_DRIVERS axd
    ON axd.IDRIVER = c.IDRIVER
   AND LOWER(LTRIM(RTRIM(axd.NATRIBUTO))) = N'dificultad'
)
MERGE dbo.CAPAC_ATRIBUTOS_PLANES AS target
USING src_attr AS src
  ON target.ICURSO = src.ICURSO
