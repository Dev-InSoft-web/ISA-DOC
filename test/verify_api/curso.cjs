...
function buildAtributosDriverNested(idriver, attrBase, jconfigSample) {
...
function verifyCursoRichResponse(data, { icurso, idriver }) {
	const a = unwrap(data);
	try {
		JSON.stringify(a);
	} catch (e) {
		console.error(`   ⛔ JSON.stringify(curso): posible referencia circular — ${e.message}`);
		return false;
	}
	const missing = [];
...
async function testDriverAtributosListSmoke(idriver, expectedIatributos, errorIcon = '⛔') {
	const checkList = (res, label) => {
		...
		const forDriver = listData.filter((r) => Number(r.idriver) === Number(idriver));
		const ids = new Set(forDriver.map((r) => r.iatributo));
		const ok = expectedIatributos.every((ia) => ids.has(ia));
		if (ok) {
			console.log(
				`   [driver/atributos] ${label}: ${forDriver.length} fila(s) idriver=${idriver}, iatributos OK ✅`,
			);
	...
	for (const ia of expectedIatributos) {
		...
		if (one.status !== 200) {
			console.error(`   GET /api/driver/atributo/${idriver}/${ia} HTTP ${one.status} ${errorIcon}`);
			console.error(`   ${JSON.stringify(unwrap(one.data)).slice(0, 400)}`);
			allOk = false;
		}
	}
	if (allOk) {
