/**
 * Registro de documentación por proyecto/derivado.
 *
 * Agrupa los archivos de `public/imgs/` (y otros recursos descargables) en
 * categorías que se renderizan como sub-pestañas dentro del panel de
 * documentación. Para añadir un documento nuevo basta con dejarlo caer en
 * `public/imgs/` y registrarlo aquí.
 */
...
/** Documentación agrupada por derivado (clave = `DERIVATIVES.key` de ProjectsPanel). */
export const DOCS_BY_DERIVATIVE: Record<string, DocCategory[]> = {
	contapymeu: [
		{
			key: "arquitectura",
			label: "Arquitectura",
...
export function getDocsForDerivative(key: string): DocCategory[] {
	return DOCS_BY_DERIVATIVE[key] ?? [];
}
