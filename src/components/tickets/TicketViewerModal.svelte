<script lang="ts">
	...
	/** Estilos inyectados en el iframe del visor (no van al correo). */
	function ticketIframeSrcdoc(html: string, onWhiteBg: boolean): string {
		const pageBg = onWhiteBg ? "#ffffff" : "transparent";
		const css = [
			`html,body{margin:0;padding:0;background:${pageBg};}`,
			"img[data-ticket-alpha=\"1\"]{background:transparent!important;}",
			".ticket-table td,.ticket-table th{border-color:#80808040;}",
		].join("");
					...
					<div class="ticket-viewer">
						...
						{:else}
							<iframe
								...
								srcdoc={ticketIframeSrcdoc(view.html, whiteBg)}
							></iframe>
						{/if}
					</div>
				{:catch err}
